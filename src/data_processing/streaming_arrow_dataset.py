#!/usr/bin/env python3
"""
StreamingArrowDataset: Strict streaming implementation following design principles
...
class StrictStreamingDataset(IterableDataset):
    """
    Strict streaming dataset that reads only one date at a time.
    Implements cross-batch sliding windows with per-sid tail buffers.
    """
    ...
    def __init__(self, data_dir: str, train_dates: List[str], test_dates: List[str],
                 ...
                 enable_immediate_processing: bool = True, batch_cleanup_threshold: int = 2):
        """
        Initialize strict streaming dataset.
        ...
        Args:
            ...
            enable_immediate_processing: Process batches immediately to save memory
            batch_cleanup_threshold: Number of batches to keep before cleanup
        """
        ...
        self.enable_immediate_processing = enable_immediate_processing
        self.batch_cleanup_threshold = batch_cleanup_threshold
        
        # LRU cache for date data (date -> DataFrame)
        ...
        logger.info(f"Strict streaming dataset initialized:")
        ...
        logger.info(f"  Immediate processing: {enable_immediate_processing}")
    ...
    def _check_memory_usage(self):
        ...
        try:
            ...
            if memory_percent > self.memory_threshold:
                logger.warning(f"Memory usage high: {memory_percent*100:.1f}%, triggering cleanup")
    ...
    def _force_memory_cleanup(self):
        ...
        with self.cache_lock:
            ...
            while len(self.data_cache) > self.cache_max_items // 2:
    ...
    def _load_date_data_strict(self, date: str) -> pd.DataFrame:
        ...
        Memory usage  approximately  batch_size  x  row_width + small tail cache.
        ...
        try:
            ...
            for batch in parquet_file.iter_batches(columns=available_cols, batch_size=self.batch_size):
                ...
                # IMMEDIATE BATCH PROCESSING: Process batches immediately to save memory
                if self.enable_immediate_processing and len(batches) > self.batch_cleanup_threshold:
    ...
    def _get_sequence_with_tail_buffer(self, stock_id: int, date: str) -> Optional[Dict]:
        """
    ...
    def _create_empty_sequence(self, stock_id: int) -> Dict:
        """Create empty sequence when data is not available."""
        feature_columns = 100  # Default feature count, adjust as needed
    ...
    def __iter__(self) -> Iterator[Dict]:
        """Iterate over sequences with strict streaming."""
        # Create date iterator with optional shuffling
        if self.enable_shuffle:
            # Buffered shuffle for large-scale streaming
            date_buffer = []
    ...
    def _iterate_date_sequences(self, date: str) -> Iterator[Dict]:
...
class RollingWindowDataLoader:
    """
    DataLoader for rolling window training with proper parallelization.
    Implements worker sharding and prefetching for IterableDataset.
    """
    ...
    def __init__(self, data_dir: str, rolling_window_years: int = 3,
                 ...
                 enable_shuffle: bool = True, shuffle_buffer_size: int = 10000):
        """
        Initialize rolling window data loader.
    ...
    def create_dataset_for_window(self, train_years: List[int], test_year: int) -> StrictStreamingDataset:
        ...
        dataset = StrictStreamingDataset(
            ...
            enable_immediate_processing=True,  # Enable immediate processing
            batch_cleanup_threshold=2  # Clean up after 2 batches
        )
    ...
    def get_data_loaders_for_window(self, train_years: List[int], test_year: int) -> Tuple[DataLoader, DataLoader]:
        ...
        Implements worker sharding for IterableDataset.
        """
        # Create datasets
